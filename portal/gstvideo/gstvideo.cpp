...
int main(int argc, char *argv[]) {
	...
	char libvisual_jess[] = "alsasrc device=hw:1 buffer-time=20000 ! queue max-size-time=50000000 leaky=upstream ! libvisual_jess ! video/x-raw,width=240,height=400 ! videoflip method=1 ! eglglessink";
	...
	char goom2k1[] = "alsasrc device=hw:1 buffer-time=20000 ! queue max-size-time=50000000 leaky=upstream ! goom2k1 ! video/x-raw,width=240,height=400 ! queue ! videoflip method=1 ! eglglessink";
	...
	char gleffects_mirror[] = "udpsrc port=9000 caps='application/x-rtp,media=(string)video,clock-rate=(int)90000,encoding-name=(string)H264' ! rtph264depay ! avdec_h264 ! videoconvert ! queue ! videoflip method=1 ! glupload ! gleffects_mirror ! gldownload ! videoflip method=3 ! eglglessink";
	...
	char revtv[] = "udpsrc port=9000 caps=application/x-rtp,media=(string)video,clock-rate=(int)90000,encoding-name=(string)H264 ! rtph264depay ! avdec_h264 ! videoconvert ! queue ! videoflip method=1 ! revtv ! videoflip method=3 ! eglglessink";
	char agingtv[] = "udpsrc port=9000 caps=application/x-rtp,media=(string)video,clock-rate=(int)90000,encoding-name=(string)H264 ! rtph264depay ! avdec_h264 ! videoconvert ! queue ! videoflip method=1 ! agingtv ! videoflip method=3 ! eglglessink";
