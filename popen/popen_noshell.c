...
void _popen_noshell_child_process(
	...
	int pipefd_0, int pipefd_1, int read_pipe, int stderr_mode,  char *file,  char *  *argv) {

	int closed_child_fd=NULL;
	int closed_pipe_fd =NULL;
	int dupped_child_fd =NULL;
...
char ** popen_noshell_copy_argv( char *  *argv_orig) {
 ...
 */
FILE *popen_noshell( char *file,  char *  *argv,  char *type, struct popen_noshell_pass_to_pclose *pclose_arg, int stderr_mode) {
	int read_pipe;
	...
	if (_popen_noshell_fork_mode) { // use fork()
		...
		arg->argv = ( char *  *)popen_noshell_copy_argv(argv);
...
int popen_noshell_fork( char *file,  char **argv,  struct popen_noshell_pass_to_pclose *pclose_arg, int stderr_mode, int pipe_in) {
	int pipefd[2]; // 0 -> READ, 1 -> WRITE ends
	pid_t pid;
	...
	if (pid == -1) return 0;
	if (pid == 0) {
		_popen_noshell_child_process(NULL, pipefd[0], pipe_in, 3, stderr_mode, file, argv);
		errx(EXIT_FAILURE, "This must never happen");
	} // child life ends here, for sure
	...
	return pipefd[1]; //return pipe out!
}
...
char ** popen_noshell_split_command_to_argv( char *command_original, char **free_this_buf) {
	char *command;
	size_t i, len;
	char *start = NULL;
	char c;
	char **argv = NULL;
	int count = 0;
	 char _popen_bash_meta_characters[] = "!\\$`\n|&;()<>";
	int in_sq = 0;
	int in_dq = 0;
	int j = 0;
#ifdef POPEN_NOSHELL_DEBUG
 ...
 * 	When you are done working with the stream, you have to close it by calling pclose_noshell(), or else you will leave zombie processes.
 */
FILE *popen_noshell_compat( char *command,  char *type, struct popen_noshell_pass_to_pclose *pclose_arg) {
	char **argv;
	FILE *fp;
	char *to_free;
	...
	fp = popen_noshell(argv[0], ( char *  *)argv, type, pclose_arg, 0);
...
int pclose_noshell2(struct popen_noshell_pass_to_pclose *arg) {
	int status;
