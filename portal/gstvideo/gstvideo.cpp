...
int main(int argc, char *argv[]) {
	...
	char libvisual_jess[] = "alsasrc device=hw:1 buffer-time=20000 ! queue max-size-time=50000000 leaky=upstream ! libvisual_jess ! video/x-raw,width=240,height=400 ! videoflip method=1 ! tee name=t ! queue ! eglglessink t. ! queue ! videorate ! video/x-raw,framerate=10/1 ! videoflip method=1 ! jpegenc ! multifilesink location=/var/www/html/tmp/snapshot.jpg";
	char libvisual_infinite[] = "alsasrc device=hw:1 buffer-time=20000 ! queue max-size-time=50000000 leaky=upstream ! libvisual_infinite ! tee name=t ! queue ! eglglessink t. ! queue  ! videorate ! video/x-raw,framerate=10/1 ! videoflip method=1 ! jpegenc ! multifilesink location=/var/www/html/tmp/snapshot.jpg";
	char libvisual_jakdaw[] = "alsasrc device=hw:1 buffer-time=20000 ! queue max-size-time=50000000 leaky=upstream ! libvisual_jakdaw ! tee name=t ! queue ! eglglessink t. ! queue ! videorate ! video/x-raw,framerate=10/1 ! videoflip method=1 ! jpegenc ! multifilesink location=/var/www/html/tmp/snapshot.jpg";
	char libvisual_oinksie[] = "alsasrc device=hw:1 buffer-time=20000 ! queue max-size-time=50000000 leaky=upstream ! libvisual_oinksie ! tee name=t ! queue ! eglglessink t. ! queue ! videorate ! video/x-raw,framerate=10/1 ! videoflip method=1 ! jpegenc ! multifilesink location=/var/www/html/tmp/snapshot.jpg";
	char goom[] = "alsasrc device=hw:1 buffer-time=20000 ! queue max-size-time=50000000 leaky=upstream ! goom ! video/x-raw,width=320,height=240 ! capssetter caps=video/x-raw,height=200 ! tee name=t ! queue ! eglglessink t. ! queue ! videorate ! video/x-raw,framerate=10/1 ! videoflip method=1 ! jpegenc ! multifilesink location=/var/www/html/tmp/snapshot.jpg";
	char goom2k1[] = "alsasrc device=hw:1 buffer-time=20000 ! queue max-size-time=50000000 leaky=upstream ! goom2k1 ! video/x-raw,width=240,height=400 ! tee name=t ! queue ! videoflip method=1 ! eglglessink t. ! queue ! videorate ! video/x-raw,framerate=10/1 ! videoflip method=1 ! jpegenc ! multifilesink location=/var/www/html/tmp/snapshot.jpg";
	...
	char revtv[] = "udpsrc port=9000 caps=application/x-rtp,media=(string)video,clock-rate=(int)90000,encoding-name=(string)H264 ! rtph264depay ! avdec_h264 ! videoconvert !  queue max-size-time=50000000 leaky=upstream ! videoflip method=1 ! revtv ! videoflip method=3 ! eglglessink";
